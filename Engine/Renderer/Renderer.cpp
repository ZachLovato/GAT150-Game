...
namespace wrap
{
	...
	void Renderer::CreateWindow(const char* name, int width, int height)
	{
		...
		m_window = SDL_CreateWindow(name, 100, 100, width, height,
			SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
		m_renderer = SDL_CreateRenderer(m_window, -2,
	...
	void Renderer::DrawLine(Vector2& v1, Vector2& v2, const Color& color)
	{
	...
	void Renderer::DrawPoint(Vector2& v, const Color& color)
	{
		SDL_SetRenderDrawColor(m_renderer, color.r, color.g, color.b, color.a);
